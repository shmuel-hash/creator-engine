        ...
        try:
            async with async_session_factory() as session:
                try:
                    ...
                    # Step 3: AI Analysis (with search mode)
                    analyzed = await analyze_results(raw_results, intent, query_text, search_mode=search_mode_val)
                    ...
                    for rd in final:
                        result = DiscoveryResult(
                            ...
                            ai_analysis={
                                ...
                                "credential_tier": rd.get("credential_tier"),
                                "medical_specialty": rd.get("medical_specialty"),
                                "practice_affiliation": rd.get("practice_affiliation"),
                                "estimated_rate": rd.get("estimated_rate"),
                                "language": rd.get("language"),
                                "country": rd.get("country"),
                                "creator_type": rd.get("creator_type"),
                                "content_niches": rd.get("content_niches", []),
...
@router.post("/discover/{search_id}/go-deeper")
async def go_deeper(search_id: UUID, db: AsyncSession = Depends(get_db)):
    """
    Run Layers 3-5 deep search on an existing doctor discovery search.
    Appends new results to the same search. Only works on doctor mode searches.
    """
    import asyncio
    ...
    if not search:
        raise HTTPException(status_code=404, detail="Search not found")
    if search.status != "complete":
        raise HTTPException(status_code=400, detail="Search must be complete before going deeper")

    # Extract topic from the original intent
    intent = search.parsed_intent or {}
    topic = intent.get("primary_niche", search.query)
    original_query = search.query

    # Mark search as running deeper
    search.status = "deep_searching"
    await db.commit()

    deep_search_id = search_id  # reuse same search
    ...
    async def _run_deep_search():
        ...
        try:
            async with async_session_factory() as session:
                try:
                    eng = DiscoveryEngine()
                    s = await session.get(DiscoverySearch, deep_search_id)
                    if not s:
                        return

                    # Generate deep search queries (Layers 3-5)
                    logger.info(f"[DeepSearch] Generating deep queries for: {original_query}")
                    deep_queries = await generate_deep_search_queries(original_query, topic)
                    logger.info(f"[DeepSearch] Generated {len(deep_queries)} deep queries")

                    s.status = "deep_searching"
                    await session.commit()

                    # Run web searches with deep queries
                    raw_results = await eng.web_search.search(deep_queries, max_results_per_query=10)
                    logger.info(f"[DeepSearch] Got {len(raw_results)} raw results from deep search")

                    s.status = "analyzing"
                    await session.commit()

                    # Analyze with doctor mode
                    analyzed = await analyze_results(raw_results, intent, original_query, search_mode="doctor")
                    logger.info(f"[DeepSearch] Analyzed {len(analyzed)} results")

                    # Deduplicate against existing results in this search
                    existing_results = await session.execute(
                        select(DiscoveryResult).where(DiscoveryResult.search_id == deep_search_id)
                    )
                    existing = existing_results.scalars().all()
                    existing_handles = set()
                    existing_names = set()
                    for er in existing:
                        if er.handle:
                            existing_handles.add(er.handle.lower().strip("@"))
                        if er.name:
                            existing_names.add(er.name.lower().strip())

                    # Filter out duplicates
                    new_results = []
                    for rd in analyzed:
                        handle = (rd.get("handle") or "").lower().strip("@")
                        name = (rd.get("name") or "").lower().strip()
                    ...
                    unique_new = deduplicate_results(new_results)
                    unique_new.sort(key=lambda x: x.get("relevance_score", 0), reverse=True)
                    final = unique_new[:20]

                    # Tag results as from deep search and store
                    for rd in final:
                        rd["source_layer"] = "deep"
                        result = DiscoveryResult(
                    ...
                    s.results_count = (s.results_count or 0) + len(final)
                    s.status = "complete"
                    s.completed_at = datetime.utcnow()
                    await session.commit()
                    logger.info(f"[DeepSearch] Complete: {len(final)} new results appended")

                except Exception as e:
                    logger.error(f"[DeepSearch] Failed: {e}", exc_info=True)
                    try:
                        s = await session.get(DiscoverySearch, deep_search_id)
                        if s:
                            s.status = "complete"  # Revert to complete so UI isn't stuck
                            s.error = f"Deep search failed: {str(e)[:300]}"
                            await session.commit()
                    except Exception:
                        pass

        except Exception as outer_e:
            logger.error(f"[DeepSearch] Background task crashed: {outer_e}", exc_info=True)

    asyncio.create_task(_run_deep_search())

    return {
        "search_id": str(search_id),
        "status": "deep_searching",
        "message": "Deep search started (Layers 3-5: LinkedIn signals, talent agencies, Reddit communities). Poll GET /discover/{search_id} for results.",
    }
